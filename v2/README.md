...
利用**汽车级薄膜压力座椅传感器**（DJ7021-5.3-11 接口）配合 micro:bit V2，检测是否有人坐在座椅上，超过设定时间后通过 LED 提醒起身活动。**v2.0.0 新增 BLE 同步功能**,可在 PC 端 Chrome 扩展查看坐了多久、剩余时间、超时报警等。
...
```
sitdetecter/
├── main.py                       # 主程序（烧录到 micro:bit，v2.0.0 蓝牙版）
├── README.md                     # 本文件
├── extension/                    # Chrome 扩展(v2.0.0 新增)
│   ├── manifest.json             #   MV3 + bluetooth 权限
│   ├── popup.html                #   圆环 + 数字 + A/B 按钮
│   ├── popup.js                  #   Web Bluetooth + 状态渲染
│   ├── popup.css                 #   深色主题
│   ├── audio/alert.mp3           #   超时提示音(< 1 秒)
│   └── README.md                 #   扩展安装说明
...
## v2.0.0 蓝牙版:Chrome 扩展

v2.0.0 在 v1.0.0 基础上新增 BLE 同步功能:micro:bit 通过自定义 GATT service 推送状态,Chrome 扩展实时显示。
...
| TX characteristic UUID(micro:bit → PC) | `6e400003-b5a3-f393-e0a9-e50e24dcca9e`,FLAG_NOTIFY |
| RX characteristic UUID(PC → micro:bit) | `6e400002-b5a3-f393-e0a9-e50e24dcca9e`,FLAG_WRITE |

**状态帧(TX,18 字节,小端序)**:

| 偏移 | 类型 | 字段 | 说明 |
|------|------|------|------|
| 0 | uint8 | state | 0=空闲,1=计时中,2=超时,3=冷却 |
| 1 | uint8 | threshold_idx | 0~3 对应 15/30/45/60 |
| 2~5 | uint32 | ref_time | micro:bit `running_time()`,毫秒 |
| 6~9 | uint32 | sit_start_ms | 本次坐下起点;0=无效 |
| 10~13 | uint32 | last_warn_ms | 上次大叉提醒时刻;0=无效 |
| 14~17 | uint32 | stand_start_ms | 站起来进入冷却的时刻;0=无效 |
| 18~21 | uint32 | actual_sit_ms | 实际坐的总时长;0=无效 |
...
4. 工具栏会出现 Sit Remoter 图标

### Chrome 扩展使用

1. 把烧录了 v2.0.0 固件的 micro:bit 上电
2. 点击工具栏图标,打开 popup
3. 点击"连接 micro:bit",在弹窗选择设备
4. 连接成功后即可看到:圆环进度、MM:SS 数字、状态文字、阈值
5. 超时会播放报警音
6. 5 秒断开自动重连
7. A/B 按钮可远程切换阈值/重置
...
- **micro:bit V2 同时只能支持一个 BLE 连接**;如果 Chrome 已连接,手机 nRF Connect 等其他客户端就连不上。
- **Chrome Web Bluetooth 不能在 Service Worker 中运行**,popup 必须保持打开才能维持连接;关闭 popup 后 5 秒自动重连。
- 状态帧 1Hz 推送,断线期间数据丢失(浏览器不补偿,重连后第一帧即新基线)。
