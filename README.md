...
- 传感器本质上是一个**常开（NO）开关**：
  - 没人坐 → 触点断开，电阻无穷大
  - 有人坐 → 触点闭合，电阻 ≤100Ω
- 利用 micro:bit P1 的**内部下拉电阻**（无需外接元件），把传感器当成"按键"：
  - 没人坐 → P1 读到 0
  - 有人坐 → P1 读到 1
...
电源
   ...
   │       │
   │       ▼
   │   ┌─── ① 红线 ── 传感器 NO 触点 ── ② 黑线 ── P1 (内部下拉) ── GND (内部)
   │   │
   │   └── 无极性:若 P1 一直读到 0,把两根线对调
   ▼
micro:bit 3.3V
```
...
|----|------|
| 接线数量 | 仅 2 根线（红 + 黑） |
| 所需外接元件 | **无**（用 P1 内部下拉电阻） |
| 极性 | **无**——两根线可对调，若 P1 一直读到 0 则反过来接 |
...
| 现象 | 可能原因 | 解决办法 |
|------|---------|---------|
| LED 一直接收不到 P1 信号（一直读到 0） | 传感器两根线接反 | 把红/黑线对调 |
...
**实时调试小贴士**：在 `while True` 开头加一行 `print("P1=", pin1.read_digital())`，然后用 Mu 的 **REPL** 或 Thonny 的串口监视器，可实时看到 P1 在有人/无人时的读数变化。
...
- 改用 P1 模拟读取 + 10kΩ 外部分压电阻，区分"轻坐 / 重坐"，给"跷二郎腿"等动作单独提醒。
